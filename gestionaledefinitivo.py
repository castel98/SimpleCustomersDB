...
#######################################################################################################
def trovapratica(pratica):
    obj1 = col.find_one({"pratica" : pratica})
    str = 'nome: ' + obj1['nome'] + ' ' + obj1['cognome'] + ', \n' + 'comune di: ' \
          + obj1['comune'] + ', \n' + 'indirizzo: ' + obj1['indirizzo'] + ', \n' + \
          'pratica: ' + obj1['pratica'] + ', \n' + 'nota: ' + obj1['nota']
    return str
...
##############################################################################################################
def trovaperpratica():
    pratica_t = tk.StringVar()


    newWindow = tk.Toplevel(app)
    newWindow.geometry("600x600")
    label_nome = tk.Label(newWindow, text="inserisci il numero di pratica da ricercare")
    label_nome.pack()
    casella_nome = tk.Entry(newWindow, textvariable=pratica_t)
    casella_nome.pack()


    def tt (pratica):
        pratica_tt = (pratica.get())
        str = trovapratica(pratica_tt)
        label= tk.Label(newWindow, text= str)
        label.pack()


    mof = partial(tt, pratica_t)
    buttonExample = tk.Button(newWindow, text="invia", command=mof)
    buttonExample.pack()
...
buttonmodi.pack()
buttontr = tk.Button(app,
              text="trova per numero di pratica",
              command=trovaperpratica)
buttontr.pack()
app.mainloop()
